...
GLOBAL_EXCLUSIONS = {"be.v.01", "be.v.03", "change.v.01", "change.v.02"}
...
def expand(word):
    ...
    for synset in wordnet.synsets(stemmed):
        for term in expand_synset(synset):
            if term not in GLOBAL_EXCLUSIONS and term not in EXCLUSIONS.get(word, []):
                yield term
...
def get_terms_from_song(song):
    ...
    tag_terms = get_terms(song.get_non_geo_tags(prefix="", exclude_prefix="artist:"))
...
class Context(object):
    ...
    def adjust_score(self, result):
        ...
        for predicate in self.predicates:
            ...
            elif predicate.applies_to_song(song, exclusive=True) and not in_context:
    ...
    def add_location_predicates(self):
        if self.configuration.location:
            locations = (
                word.lower() for word in self.configuration.location.split(",")
            )
            self.predicates.extend([String(location) for location in locations])
...
class Terms(Predicate):
    ...
    def __init__(self, ne_expanded_terms=frozenset()):
        ...
        ne_expanded_terms = self.non_exclusive_terms_expanded | ne_expanded_terms
        if self.terms_expanded or ne_expanded_terms:
            ...
            self.non_exclusive_terms_expanded = frozenset(
                get_terms(self.non_exclusive_terms)
            )
    ...
    def get_intersection(self, song_terms, exclusive=True):
        ...
        return song_terms & (self.terms_expanded | self.non_exclusive_terms_expanded)

    def applies_to_song(self, song, exclusive):
        return self.regex_terms_match(song, exclusive) or self.get_intersection(
...
class Period(ExclusiveTerms):
    ...
    def get_factor(self, song, exclusive=False):
        ...
        return factor * (
            1.0 - min(1, (self.diff.total_seconds() / self.decay.total_seconds()))
        )
...
class Snow(Weather):
    ...
    non_exclusive_terms_expanded = Cloudy.non_exclusive_terms_expanded | frozenset(
        [
            u"snow.n.01",
            u"snow.n.02",
            u"snow.n.03",
            u"precipitation.n.03",
...
class Season(Period):
    ...
    def __init__(self, peak):
        self.terms_expanded = frozenset(["%s.n.01" % self.__class__.__name__.lower()])
...
class Night(Period):
    ...
    period = ONE_DAY
    decay = SIX_HOURS
    terms_expanded = frozenset(["night.n.01", "night.n.02", "night.n.03", "night.n.07"])
...
class Evening(Period):
    ...
    period = ONE_DAY
    decay = THREE_HOURS
    terms_expanded = frozenset(["evening.n.01", "evening.n.02", "evening.n.03"])
...
class Halloween(Period):
    ...
    non_exclusive_terms_expanded = frozenset(
        [
            ...
            u"die.v.10",
            u"hex.v.01",
            u"mythical_monster.n.01",
            u"occultist.n.01",
            u"nefariousness.n.01",
            u"werewolf.n.01",
            u"mutant.a.01",
            u"death.n.03",
            u"death.n.06",
            ...
            u"bat.n.05",
            u"bat.n.01",
            u"terrorization.n.01",
            u"transgression.n.01",
            u"kill.v.04",
            u"kill.v.09",
            u"skull.n.01",
        ]
...
class Ascension(EasterBased):
    ...
    terms_expanded = frozenset(["ascension.n.03", u"ascension.n.01", u"ascension.n.04"])
    non_exclusive_terms_expanded = frozenset(
...
class Assumption(Date):
    ...
    non_exclusive_terms_expanded = frozenset(
        [u"miracle.n.02", u"heaven.n.02", u"holy_day_of_obligation.n.01", u"mary.n.01"]
    )
...
class GroundhogDay(Date):
    ...
    non_exclusive_terms_expanded = frozenset([u"marmot.n.01", u"groundhog.n.01"])
...
class Birthday(Date):
    def __init__(self, birth_date, name, age):
        self.non_exclusive_terms = ("birthday", name, str(birth_date.year), str(age))
